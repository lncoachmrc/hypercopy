...
function Shell(){const {user,signOut}=useAuth();return <div><header><BrandLogo/><nav><NavLink to="/">Dashboard</NavLink><NavLink to="/settings">Configurazione</NavLink><NavLink to="/billing">Piano</NavLink>{user&&['ADMIN','SUPERADMIN'].includes(user.role)&&<NavLink to="/admin">Control room</NavLink>}</nav><div style={{display:'flex',alignItems:'center',gap:10}}><LanguageSelector compact/><AiModeToggle/><AiStatus/><button onClick={()=>void signOut()}>Esci</button></div></header><div className="wrap"><Routes><Route index element={<Dashboard/>}/><Route path="settings" element={<Settings/>}/><Route path="billing" element={<Billing/>}/><Route path="admin" element={<Gate admin><><Admin/><AdminPlanDiscounts/></></Gate>}/></Routes></div></div>}
