...
function Shell(){const {user,signOut}=useAuth();const {disconnect}=useDisconnect();const logout=async()=>{try{await disconnect();}catch{}await signOut();};return <div><header><BrandLogo/><nav><NavLink to="/">Dashboard</NavLink><NavLink to="/settings">Configurazione</NavLink><NavLink to="/billing">Piano</NavLink>{user&&['ADMIN','SUPERADMIN'].includes(user.role)&&<NavLink to="/admin">Control room</NavLink>}</nav><div style={{display:'flex',alignItems:'center',gap:10}}><LanguageSelector compact/><AiModeToggle/><AiStatus/><button onClick={()=>void logout()}>Esci</button></div></header><div className="wrap"><Routes><Route index element={<MasterAwareDashboard/>}/><Route path="settings" element={<MasterAwareSettings/>}/><Route path="billing" element={<Billing/>}/><Route path="admin" element={<Gate admin><><Admin/><AdminPlanDiscounts/></></Gate>}/></Routes></div></div>}
