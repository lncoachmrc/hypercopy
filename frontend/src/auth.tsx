...
import {get,post,setCsrf,type Session,type SessionUser} from './api';
...
export function AuthProvider({children}:{children:ReactNode}){
 ...
 const refresh=useCallback(async()=>{try{const s=await get<Session>('/auth/session');setCsrf(s.csrf_token);setUser(s.user);setEnt(s.entitlements);}catch{setUser(null);setEnt({});}finally{setLoading(false);}},[]);
