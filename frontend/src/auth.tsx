...
export function AuthProvider({children}:{children:ReactNode}){
 ...
 const refresh=useCallback(async()=>{setError(null);try{const s=await get<Session>('/auth/session');setCsrf(s.csrf_token);setUser(s.user);setEnt(s.entitlements);}catch(e){setUser(null);setEnt({});if(e instanceof ApiError&&e.status===401){setError(null);}else{setError(e instanceof Error?e.message:'Impossibile contattare TRAXION.');}}finally{setLoading(false);}},[]);
