...
export function AuthProvider({children}:{children:ReactNode}){
 ...
 const applySession=useCallback((s:Session)=>{setCsrf(s.csrf_token);setUser(s.user);setEnt(s.entitlements);},[]);
 const refresh=useCallback(async()=>{setError(null);try{const s=await get<Session>('/auth/session');applySession(s);}catch(e){clearSession();if(e instanceof ApiError&&e.status===401){setError(null);}else{setError(e instanceof Error?e.message:'Impossibile contattare TRAXION.');}}finally{setLoading(false);}},[applySession,clearSession]);
 useEffect(()=>{
