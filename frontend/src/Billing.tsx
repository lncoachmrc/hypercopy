...
export default function Billing(){
  ...
  return <>
    <div className="title"><div><h1>Piani TRAXION</h1><p>Il piano dipende dal valore del portafoglio operativo. Tutti i limiti sono applicati anche lato backend.</p></div>{s?.plan&&s.plan!=='trial'&&<button onClick={()=>void portal()}>Customer Portal</button>}</div>
    ...
    <section className="panel billing-current"><div><span>Piano attuale</span><strong>{current==='starter'?'Starter':current==='plus'?'Plus':current==='pro_10k'?'Pro':s?.plan||'—'}</strong></div><div><span>Stato</span><strong>{s?.status||'—'}</strong></div><div><span>Entitlement</span><strong className={s?.entitled?'up':'down'}>{s?.entitled?'ATTIVO':'NON ATTIVO'}</strong></div>{s?.portfolio_equity!=null&&<div><span>Portafoglio rilevato</span><strong>{usd(s.portfolio_equity,2)}</strong></div>}</section>
    ...
    <div className="pricing-grid">{catalog?.plans.map(p=>{
      ...
      return <section className={`pricing-card ${p.slug==='plus'?'featured':''}`} key={p.slug}>
        ...
        <div className="pricing-billed">{annual?`fatturato ${usd(billed,moneyDecimals(billed))}/anno`:'fatturato mensilmente'}{personalDiscount>0?' · sconto applicato automaticamente al checkout':''}</div>
        ...
        <button className="pricing-cta" disabled={isCurrent||!configured||busy===p.slug} title={!configured?'Configura il relativo Stripe Price ID per abilitare il checkout':''} onClick={()=>void checkout(p.slug)}>{isCurrent?'Piano attuale':busy===p.slug?'Apertura checkout…':configured?'Scegli piano':'Checkout da configurare'}</button>
      </section>})}</div>
