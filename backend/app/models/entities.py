...
class PositionLedger(BaseUuid, Base):
    ...
    master_leverage: Mapped[int | None] = mapped_column(Integer)
    master_is_cross: Mapped[bool | None] = mapped_column(Boolean)
    follower_leverage: Mapped[int | None] = mapped_column(Integer)
    follower_is_cross: Mapped[bool | None] = mapped_column(Boolean)
...
class RISExExecutionControl(Base):
    __tablename__ = 'risex_execution_control'
    ...
    target_worker_id: Mapped[str] = mapped_column(String(80), nullable=False)
    target_boot_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False)
    ...
    consumed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    consumed_by_worker_id: Mapped[str | None] = mapped_column(String(80))
    consumed_by_boot_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True))
    supersedes_request_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey('risex_execution_control.request_id', ondelete='SET NULL'))
    readiness_assertions: Mapped[dict] = mapped_column(JSON, default=dict, nullable=False)
    __table_args__ = (
        Index(
        ...
        Index('ix_risex_execution_control_requested_at', 'requested_at'),
    )
