...
def test_hash_ip_uses_versioned_dedicated_hmac_and_not_session_secret(monkeypatch) -> None:
    key = b'k' * 32
    ip = '203.0.113.42'
    monkeypatch.setattr(security.settings, 'AUDIT_IP_HASH_KEY_B64', _key_b64(key), raising=False)
...
def test_hash_ip_distinguishes_ips(monkeypatch) -> None:
    monkeypatch.setattr(security.settings, 'AUDIT_IP_HASH_KEY_B64', _key_b64(b'k' * 32), raising=False)
