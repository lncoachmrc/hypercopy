...
def upgrade():
    # Use the driver-level execution path for static JSON literals. SQLAlchemy
    # text() interprets JSON ':' characters as bind parameter markers.
    bind = op.get_bind()
    bind.exec_driver_sql(
        """INSERT INTO plans (slug,name,active,limits) VALUES
    ...
    bind.exec_driver_sql(
        """INSERT INTO system_flags (slug,enabled,value,reason) VALUES
...
def downgrade():
    bind = op.get_bind()
    bind.exec_driver_sql(
        "DELETE FROM system_flags WHERE slug IN ('live_trading','global_pause','emergency_stop')"
    )
    bind.exec_driver_sql(
        "DELETE FROM plans WHERE slug IN ('trial','basic','pro','enterprise')"
    )
