...
3. immediately before the provider POST, inside the submission boundary, revalidate that the job's exact `ExecutionEpoch` is still the user's active epoch and that its `credential_version` still matches the generation of the currently stored RISEx signing credential. A credential superseded by credential rotation or by a destination change must never be used for a POST, even if its on-chain session remains active. A concurrency test must demonstrate this property by completing credential rotation and destination-epoch change between credential resolution and the would-be POST and proving that submission is blocked;
